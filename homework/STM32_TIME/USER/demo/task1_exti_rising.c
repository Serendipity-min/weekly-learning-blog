/**
 ******************************************************************************
 * @file    task1_exti_rising.c
 * @brief   任务 1 - 上升沿触发中断实验 (EXTI_Trigger_Rising)
 * 
 * 硬件连接与原理（KEY0 -> PE4）：
 ...
 * 
 * 预期实验现象：
 * 1. 按下 KEY0 时：产生下降沿，不满足上升沿触发条件，无中断响应，LED0 状态不变。
 * 2. 松开 KEY0 时：产生上升沿，触发 EXTI4 中断，LED0 状态翻转，g_exti_count 自增 1。
 * 3. 结论：由于 KEY0 按下接 GND（低电平有效），上升沿触发对应的是【松手瞬间】！
 ******************************************************************************
 */
...
/**
 * @brief 任务 1-1 初始化函数 (KEY0 -> PE4 -> EXTI4 上升沿触发)
 */
void Task1_Rising_Init(void)
{
    ...
    EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Rising; /* 上升沿触发 (对应 KEY0 松手) */

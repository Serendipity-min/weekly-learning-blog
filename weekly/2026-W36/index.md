...
1. **输入保护二极管**：两只钳位二极管分别连接至 VDD 与 VSS。当引脚外部输入电压高于 `VDD + 0.3V` 时上方二极管导通分流，当低于 `VSS - 0.3V` 时下方二极管导通，保护内部核心晶体管不被静电与反向尖峰击穿；
...
> 因此：
> - 挂载在 APB1（42MHz）上的通用/基本定时器（TIM2~TIM7、TIM12~TIM14）实际时钟为 `42MHz * 2 = 84MHz`；
> - 挂载在 APB2（84MHz）上的高级定时器（TIM1、TIM8）实际时钟为 `84MHz * 2 = 168MHz`。
...
- **整板静态与工作功耗实测**：红表笔接入 mA 档，黑表笔接 COM，将万用表**串联**接入开发板 5V 供电回路中。实测 STM32F407 核心小系统正常运行功耗约为 **30mA @ 5V**（整板静态功率约 `0.15W`）；
- **硬件焊接故障定位秘籍**：
...
- **AUTO 自动设置门限**：示波器 AUTO 按键自动搜索信号的硬件门限要求待测信号频率 `>= 50Hz` 且占空比 `> 1%`。
...
挂载在 APB1 总线上的 TIM6/TIM7 输入时钟频率为 `Tclk = 84MHz`。
...
**实战算例（定时精准 1 秒闪烁 LED）**：
- 输入时钟：`Tclk = 84MHz = 84,000,000Hz`
- 设置预分频系数：`PSC = 8400 - 1 = 8399`，分频后计数器时钟频率为：
  ```text
  fcnt = 84,000,000 / 8400 = 10,000Hz  (即每计一次数为 0.1ms)
  ```
- 设置自动重装载值：`ARR = 10000 - 1 = 9999`
- 则溢出时间为：
  ```text
  Tout = (10000 * 8400) / 84,000,000Hz = 1.0s
  ```
...
- **最大定时时间推导**：
  ...
  ```text
  Tmax = (2^24) / 168,000,000Hz = 16,777,216 / 168,000,000 ≈ 0.09986s ≈ 99.86ms
  ```
  若改用 8 分频外部参考时钟（`168MHz / 8 = 21MHz`），最大定时时间可扩展至约 **798.9ms**。
...
2. **学习 I2C / SPI 硬件控制器与外设芯片驱动**：实操 OLED 显示屏（SSD1306）与 SPI Flash（W25Q64）驱动开发；
3. **FreeRTOS 实时操作系统引入**：从裸机轮询/中断架构平滑过渡到多任务实时抢占内核。
